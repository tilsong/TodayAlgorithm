...
//    }
    ...
    private static void dfs(int r, int c, int dir) {
        ...
        // 사방으로 청소할 곳 있는지 찾고 있으면 처리
        for (int i = 0; i < 4; i++) {
            dir = (dir + 3) % 4;

            int nRow = r + dRow[dir];
            int nCol = c + dCol[dir];
            if (nRow >= 0 && nCol >= 0 && nRow < n && nCol < m && room[nRow][nCol] == 0) {
                count++;
                dfs(nRow, nCol, dir);
                return;
            }
        ...
        // 없으면 후진 여부 확인 후 후진되면 다시 처리
        int back = (dir + 2) % 4;
        int bRow = r + dRow[back];
        int bCol = c + dCol[back];

        if (bRow >= 0 && bCol >= 0 && bRow < n && bCol < m && room[bRow][bCol] != 1) {
            dfs(bRow, bCol, dir);
        }
    }
}
