...
public class 개똥벌레 {
	...
	//			주요 포인트였다고 생각한다.
	//			이분 탐색으로 푼 풀이도 봤는데, 누적합 풀이가 더 나은 것 같다.
	//			가끔 이분탐색 분류인데 이분탐색으로 했을 때보다 더 나은 효율을 보이는 풀이법들이 있다.
	//			사실 다른 문제들에서도 그런 경우가 많은데, 그럴 때 과감하게 풀이 방식을 선택하여 푸는 것도 나쁘지 않다고 생각한다.
	//			각 문제는 적절한 풀이법이 있는 것이기 때문에, 굳이 해당 분류 풀이법을 사용하려고 더 고생할 필요는 없다고 생각한다.
	//			그리고 보통 그런 경우 못 풀었다.
